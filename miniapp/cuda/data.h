...
class Field {
	...
	auto size_in_bytes = xdim_ * ydim_ * sizeof(double);
	auto status = cudaMemcpy(host_ptr_, device_ptr_, size_in_bytes, cudaMemcpyDeviceToHost);
	...
	auto size_in_bytes = xdim_ * ydim_ * sizeof(double);
        auto status = cudaMemcpy(device_ptr_, host_ptr_, size_in_bytes, cudaMemcpyHostToDevice);
        cuda_check_status(status);
    }
